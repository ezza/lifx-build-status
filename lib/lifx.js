...
function lifx(event, data) {
  ...
  if (event == 'status' && data.context == 'ci/circleci') {
    ...
    switch (data.state) {
      case 'failure':
        if (defaultBranch) {
          incoming(event + ': build failure @ ' + secrets.defaultBranch);
          pulsate(Color.Red + Brightness.High, true);
        } else {
          incoming(event + ': build failure @ feature');
      ...
      case 'success':
        if (defaultBranch) {
          incoming(event + ': build success @ ' + secrets.defaultBranch);
          flash(Color.Green + Brightness.High);
        } else {
          incoming(event + ': build success @ feature');
...
function incoming(str) {
  console.log('Incoming webhook for ' + str);
}
